import { COLORS, type LinhaZeroCase } from "@/lib/case-model";
import {
  TabIdentificacao,
  TabAnamnese,
  TabMenopausa,
...
export function CaseForm({
  ...
  const readOnly = editing.visitLocked;
  return (
    <div className="bg-white rounded-lg border border-slate-200 overflow-hidden">
      <div
        className="px-6 py-4 border-b border-slate-200 flex flex-wrap items-center justify-between gap-2"
        style={{ backgroundColor: COLORS.accent }}
      >
        <div>
          <h2 className="font-bold text-slate-800">
            {isNew ? "Novo caso" : "Editar caso"} ·{" "}
            {editing.studyCode || editing.codigo || "(sem código)"}
          </h2>
          {readOnly && (
            <p className="text-xs text-amber-700 mt-1">
              Visita T0 bloqueada — somente leitura
            </p>
          )}
        </div>
        <div className="flex gap-2">
          ...
          <button
            ...
            className="px-4 py-1.5 rounded text-white font-medium text-sm disabled:opacity-50"
