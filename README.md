...
| Ficheiro | Uso |
|----------|-----|
| `railway.toml` | [Railway](https://railway.app): build via **`deploy/Dockerfile`**, start `npm start`, health `/api/health`. |
| `render.yaml` | [Render](https://render.com): Blueprint com os mesmos comandos. |
| `deploy/Dockerfile` | Imagem de produção (Railway e `docker build -f deploy/Dockerfile`). |
